...
from keras import backend as K  # 后端用K
...
# 进行配置，使用30%的GPU
connn = tf.ConfigProto()
connn.gpu_options.per_process_gpu_memory_fraction = 0.2
session = tf.Session(config=connn)

# 设置session
KTF.set_session(session)

# limite the recursion numbers
sys.setrecursionlimit(40000)

# add some arguments for runing
parser = OptionParser()
...
if options.parser == 'pascal_voc':
	from keras_frcnn.CCF_BD_parser import get_data  # 此处做了修改
...
C = config.Config()  # 里面都是关于网络的一些参数预设定
...
print("进入程序")
# 以下代码主要是对文件运行时所指定的参数的鉴别
# 查看默认使用的网络，如果没有指定vgg使用resnet50
if options.network == 'vgg':
	C.network = 'vgg'
	from keras_frcnn import vgg as nn
elif options.network == 'resnet50':
	from keras_frcnn import resnet as nn  # 不同网络 nn代表的网络（文件）不同
...
all_imgs, classes_count, class_mapping = get_data(options.train_path)  # 在这里获取了信息

if 'bg' not in classes_count:  # 检测一下统计的类别中是否有background=bg这个分类，没有的话记得设置以下
	classes_count['bg'] = 0
	class_mapping['bg'] = len(class_mapping)

C.class_mapping = class_mapping

inv_map = {v: k for k, v in class_mapping.items()} # 把每一个类对应一个数字 保存在inv_map中

print('Training images per class:')
pprint.pprint(classes_count)  # 输出类别以及每个类别对应的个数
print('Num classes (including bg) = {}'.format(len(classes_count)))  # 输出一共有多少个类

config_output_filename = options.config_filename

with open(config_output_filename, 'wb') as config_f:
	pickle.dump(C, config_f)  # 将前面设置的参数保存到config_output_filename，既config.pickle文件中
	print('Config has been written to {}, and can be loaded when testing to ensure correct results'.format(
		config_output_filename))

random.shuffle(all_imgs)  # 随机排序

num_imgs = len(all_imgs)  # 一共有多少个图片

# train_imgs = [s for s in all_imgs if s['imageset'] == 'trainval'] #有trainval标签的拿去训练
# val_imgs = [s for s in all_imgs if s['imageset'] == 'test']

train_imgs = all_imgs  ####这里让我修改了
val_imgs = []
...
model_all = Model([img_input, roi_input], rpn[:2] + classifier)  # 构造整体网络，最后输出用于最终的分类 + 也兼顾训练PRN中的两个子层（全连接层） ？
...
epoch_length = 1000
num_epochs = int(options.num_epochs)  # 默认2000
iter_num = 0
...
for epoch_num in range(num_epochs):
	...
	while True:
		try:
			...
			# print(len(img_data))
			# print(len(img_data))
			# a, s, d = next(data_gen_train)
			# q, w, e = next(data_gen_train)
			# X = [a, q]
			# Y = [s, w]
			# img_data = (d, e)
			# print(type(X))           #<type 'numpy.ndarray'>
			# print(type(Y))           #<type 'list'>
			# print(type(img_data))    #<type 'dict'>
