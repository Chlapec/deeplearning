...
def get_data(input_path):
	...
	for data_path in data_paths:
		...
		annot_path = os.path.join(data_path, 'Annotations')  #图像注释路径
		imgs_path = os.path.join(data_path, 'JPEGImages')    #原始图片路径
		...
		test_files = []
		try:
			with open(imgsets_path_trainval) as f: #将ImageSets/Main/trainval.txt（其实这里面标记了可以拿去训练，既作训练集的图片的名称）打开，其中的每一行加上.jpg存入trainval_files
				for line in f:
		...
		try:
			...
			if data_path[-7:] == 'VOC2012':   #VOC2012是没有这个训练文件的，所以使用2012的时候确实没有报错
				# this is expected, most pascal voc distibutions dont have the test.txt file
				pass
			else:
				print(e)
		
		annots = [os.path.join(annot_path, s) for s in os.listdir(annot_path)] #返回这个目录下多有文件的一个列表
		idx = 0                          #一个索引
		for annot in annots:             #Annotations中的每一个文件（对应一个图片的bounding box坐标）
			try:
				...
				if len(element_objs) > 0: # >0说明确实有 对象
					annotation_data = {'filepath': os.path.join(imgs_path, element_filename), 'width': element_width, #一个annotation_data是一个图片中的各种数据的字典
									   'height': element_height, 'bboxes': []}  #根据xml中获取的element_filename结合前面的imgs_path,可以在原始图像目录中定位一张图片
				...
				for element_obj in element_objs:
					class_name = element_obj.find('name').text  #获取 对象 的name 比如：person、horse等
					...
					if class_name not in class_mapping:         #如果不在mapping中的情况
						class_mapping[class_name] = len(class_mapping) #给检索到的类别 进行排序

					obj_bbox = element_obj.find('bndbox')       #获取boundingbox坐标
					...
					annotation_data['bboxes'].append(
						{'class': class_name, 'x1': x1, 'x2': x2, 'y1': y1, 'y2': y2, 'difficult': difficulty}) #在这里终于将最后的bboxes条目补充满了
